        ...
        child: SafeArea(child: Column(children: [
          ...
          Container(
            ...
            child: TabBar(
              ...
              tabs: const [Tab(text: 'Veo 3 ✨'), Tab(text: 'Seedance'), Tab(text: 'Kilwa')],
            ),
          ),
          Expanded(child: TabBarView(controller: _tabCtrl, children: const [_Veo3Tab(), _SeedanceTab(), _KilwaVideoTab()])),
        ])),
      ),
    );
  }
...
// ── Veo 3 Tab (NEW) ────────────────────────────────────────────────────────
class _Veo3Tab extends StatefulWidget {
  const _Veo3Tab();
  @override
  State<_Veo3Tab> createState() => _Veo3TabState();
}

class _Veo3TabState extends State<_Veo3Tab> {
  final _ctrl = TextEditingController();
  bool _loading = false;
  String? _resultUrl;
  String? _error;
  String? _inputImageUrl;
  String _model = 'veo-3.1';
  String _aspectRatio = '16:9';
  int _pollStep = 0;
  ...
  Future<void> _pickImage() async {
    ...
    try {
      final url = await StorageService().uploadMedia(file, 'ai_temp');
      if (mounted) setState(() => _inputImageUrl = url);
    } catch (_) { if (mounted) setState(() => _error = 'فشل تحميل الصورة.'); }
  ...
  Future<void> _generate() async {
    ...
    if (prompt.isEmpty || _loading) return;
    final userId = context.read<AppProvider>().currentUser?.id ?? '';
    setState(() { _loading = true; _resultUrl = null; _error = null; _pollStep = 0; });

    // Show poll progress
    final timer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (mounted && _loading) setState(() => _pollStep++);
    });

    try {
      final url = await AiService().veoGenerate(
        prompt: prompt, uid: userId,
        model: _model, aspectRatio: _aspectRatio,
      ...
      timer.cancel();
      if (mounted) setState(() { _loading = false; _pollStep = 0; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        GlassContainer(padding: const EdgeInsets.all(14), child: Column(children: [
          Row(children: [
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
              decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF7B1FA2), Color(0xFF4A148C)]), borderRadius: BorderRadius.circular(8)),
              child: const Text('✨ NEW', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900)),
            ),
            const SizedBox(width: 10),
            const Expanded(child: Text('Veo 3.1 - أفضل فيديو AI في العالم', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 13))),
          ]),
          const SizedBox(height: 8),
          const Text('• يدعم الصوت الحقيقي في الفيديو\n• جودة سينمائية 4K\n• نص إلى فيديو وصورة إلى فيديو',
            style: TextStyle(color: AppColors.textMuted, fontSize: 12, height: 1.6)),
        ])),
        const SizedBox(height: 12),

        // Model selector
        Row(children: AppConstants.veoModels.asMap().entries.map((e) => Expanded(child: Padding(
          padding: EdgeInsets.only(right: e.key == 0 ? 8 : 0),
          child: GestureDetector(
            onTap: () => setState(() => _model = e.value),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 180),
              padding: const EdgeInsets.symmetric(vertical: 10),
              decoration: BoxDecoration(
                gradient: _model == e.value ? AppGradients.accentGradient : null,
                color: _model == e.value ? null : AppColors.bgLight,
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: _model == e.value ? AppColors.accent : AppColors.glassBorder),
              ),
              child: Text(e.value, textAlign: TextAlign.center,
                style: TextStyle(color: _model == e.value ? Colors.white : AppColors.textSecondary, fontWeight: FontWeight.w700, fontSize: 13)),
            ),
          ),
        ))).toList()),
        const SizedBox(height: 12),
        ...
        Text('نسبة العرض', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 13, fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        Row(children: AppConstants.veoRatios.asMap().entries.map((e) => Expanded(child: Padding(
          padding: EdgeInsets.only(right: e.key == 0 ? 8 : 0),
          child: GestureDetector(
            onTap: () => setState(() => _aspectRatio = e.value),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 180),
              padding: const EdgeInsets.symmetric(vertical: 10),
              decoration: BoxDecoration(
                gradient: _aspectRatio == e.value ? AppGradients.accentGradient : null,
                color: _aspectRatio == e.value ? null : AppColors.bgLight,
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: _aspectRatio == e.value ? AppColors.accent : AppColors.glassBorder),
              ),
              child: Text(e.value, textAlign: TextAlign.center,
                style: TextStyle(color: _aspectRatio == e.value ? Colors.white : AppColors.textSecondary, fontWeight: FontWeight.w700, fontSize: 13)),
            ),
          ),
        ))).toList()),
        const SizedBox(height: 12),
        ...
        // Image-to-video
        GestureDetector(
          onTap: _inputImageUrl == null ? _pickImage : () => setState(() => _inputImageUrl = null),
          child: AnimatedContainer(
            duration: const Duration(milliseconds: 180),
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            decoration: BoxDecoration(
              gradient: _inputImageUrl != null ? AppGradients.accentGradient : null,
              color: _inputImageUrl == null ? AppColors.bgLight : null,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: _inputImageUrl != null ? AppColors.accent : AppColors.glassBorder),
            ),
            child: Row(children: [
              Icon(_inputImageUrl != null ? Icons.image_rounded : Icons.add_photo_alternate_outlined,
                  size: 20, color: _inputImageUrl != null ? Colors.white : AppColors.textMuted),
              const SizedBox(width: 10),
              Text(_inputImageUrl != null ? 'صورة محددة - اضغط للإزالة' : 'أضف صورة (Image → Video)',
                  style: TextStyle(color: _inputImageUrl != null ? Colors.white : AppColors.textMuted, fontSize: 14, fontWeight: FontWeight.w600)),
            ]),
          ),
        ),
        const SizedBox(height: 12),

        TextField(controller: _ctrl, maxLines: 3, style: const TextStyle(color: Colors.white, fontSize: 15),
          decoration: const InputDecoration(hintText: 'صف الفيديو: منظر سينمائي لمدينة مستقبلية...')),
        const SizedBox(height: 16),
        ...
        ElevatedButton.icon(
          ...
          label: Column(mainAxisSize: MainAxisSize.min, children: [
            Text(_loading ? 'جاري المعالجة...' : 'توليد بـ Veo 3',
                style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
            if (_loading && _pollStep > 0)
              Text('خطوة $_pollStep من 48 - قد يستغرق 4 دقائق',
        ...
        if (_loading && _pollStep > 0) ...[
          ...
          ClipRRect(borderRadius: BorderRadius.circular(6),
            child: LinearProgressIndicator(
              value: _pollStep / 48.0, backgroundColor: AppColors.bgLight,
              color: const Color(0xFF7B1FA2), minHeight: 4)),
        ],
        ...
        if (_resultUrl != null) ...[
          const SizedBox(height: 16),
          GlassContainer(padding: const EdgeInsets.all(8), child: Column(children: [
            VideoPlayerWidget(url: _resultUrl!),
            const SizedBox(height: 8),
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.check_circle_rounded, size: 16, color: AppColors.online),
              const SizedBox(width: 6),
              const Text('تم التوليد بنجاح - Veo 3.1', style: TextStyle(color: AppColors.online, fontSize: 13)),
            ]),
          ])),
        ],
      ]),
