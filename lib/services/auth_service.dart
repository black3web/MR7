...
class AuthService {
  ...
  Future<void> toggleBan(String userId) async {
    try {
      ...
      await _db.collection(AppConstants.colUsers).doc(userId).update({'isBanned': !isBanned});
    } catch (e) { 
      debugPrint('[Auth] toggleBan error: $e');
      throw Exception('فشل تغيير حالة الحظر'); 
    }
  }
  ...
  Future<void> toggleAdmin(String userId) async {
    try {
      ...
      await _db.collection(AppConstants.colUsers).doc(userId).update({'isAdmin': !isAdmin});
    } catch (e) { 
      debugPrint('[Auth] toggleAdmin error: $e');
      throw Exception('فشل تغيير صلاحية المشرف'); 
    }
  }
}
