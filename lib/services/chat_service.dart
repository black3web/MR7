...
class ChatService {
  ...
  /// Wrapper that sends a message AND creates a notification for the recipient
  Future<MessageModel> sendMessageWithNotif({
    required String chatId,
    required String senderId,
    required String senderName,
    required String? senderPhotoUrl,
    ...
    if (recipientId != null && recipientId != senderId) {
      String body;
      switch (type) {
        case MessageType.image:  body = '📷 أرسل صورة'; break;
        case MessageType.video:  body = '🎥 أرسل فيديو'; break;
        case MessageType.voice:  body = '🎤 رسالة صوتية'; break;
        case MessageType.sticker:body = '🙂 ملصق'; break;
        default: body = text ?? '';
      }
      try {
  ...
  Future<String?> getOtherUserId(String chatId, String myId) async {
    try {
  ...
  Future<void> setTyping(String chatId, String userId, bool isTyping) async {
    try {
      await _firestore.collection(AppConstants.colChats).doc(chatId).update({
        'typingUsers.$userId': isTyping ? true : FieldValue.delete(),
      });
    } catch (_) {}
  }

  Stream<Map<String, dynamic>> typingStream(String chatId) =>
      _firestore.collection(AppConstants.colChats).doc(chatId).snapshots()
          .map((d) => (d.data()?['typingUsers'] as Map<String, dynamic>?) ?? {});
}
